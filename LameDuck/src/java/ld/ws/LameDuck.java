...
import lameduck.web.GetCancelFault;
...
import lameduck.web.CancelFaultType;
import lameduck.web.CancelRequest;
...
public class LameDuck {
    ...
    public boolean cancelFlight(lameduck.web.CancelRequest getCancelRequest) throws GetCancelFault {
        //TODO implement this method
        CancelRequest request = getCancelRequest;
        String bookingNum = request.getBookingNumber();
        ...
        try {
            // Refund the given Credit Card with 50% of the price
            boolean answer = refundCreditCard(4, creditCardCopy, (int) (price*0.5), acc);
            return answer;  // Return the answer from the web service
        } catch (CreditCardFaultMessage ex) {
            // Return a special fault 
            GetCancelFault refundFault;
            CancelFaultType faultRefund = new CancelFaultType();
            faultRefund.setErrorMessage(ex.getFaultInfo().getMessage());
